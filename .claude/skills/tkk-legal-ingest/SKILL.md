...
| **lint** | "lint检查"、"健康检查" | 断链/矛盾/过时/空白/源文件完整性 | [[references/lint-workflow.md]] |
...
> 详细内容见 [[references/file-types.md]]
...
| 6 | **行政程序规范** | **下级行政机关（司/处/科/县区级）** | **internal_norm** |

**文件类型与目标目录速查**：

| 文件类型 | 目标目录 | type | 保留度 |
|----------|----------|------|--------|
| 律师业务指引 | `wiki/summaries/` | `summary` | 摘录要点 |
| 法律法规 | `wiki/concepts/` | `concept` | 摘录要点 |
| 司法解释原文 | `wiki/entities/` | `entity` | **100%** |
| 行政法规原文 | `wiki/entities/` | `entity` | **100%** |
| 部门规章 / 地方政府规章 | `wiki/entities/` | `entity` | **100%** |
| **行政程序规范** | `wiki/entities/` | `entity` | **100%** |
| 理解与适用 | `wiki/entities/`或`concepts/` | `entity`或`concept` | 摘录要点 |
| 案例分析 | `wiki/syntheses/` | `synthesis` | 摘录要点 |

**判断规则**：文件名含"指引"/"指南"/"规程"→summaries；含"解释"/"规定"→entities；含"案例"/"指导案例"→syntheses；**其他→读前20行确认**
...
| [[references/query-workflow.md]] | query 操作详细流程（含快速回答框架 + 刑事研究专用8步流程） |
