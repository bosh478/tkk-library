#!/usr/bin/env python3
"""Extract chapters from 法医病理司法鉴定实务 into separate files - fixed version."""
...
for idx, (ln, name) in enumerate(chapters):
    print(f"  {idx+1}. {name} at line {ln+1}")

# Helper function to convert Chinese numbers to Arabic
...
for idx in range(len(chapters)):
    ...
    chapter_num = cn_to_arabic(chapter_name[1:])  # Extract number part and convert
...
# 一句话说明
法医病理司法鉴定实务第{chapter_num}章，系统阐述{chapter_name.replace('章', '')}的理论与实践。
