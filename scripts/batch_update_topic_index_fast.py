#!/usr/bin/env python3
"""Fast batch update topic_index using grep with multiple keywords"""
import json
import os
import subprocess
import sys
...
# VAULT_ROOT 多平台兼容：
# 1. 优先使用 TKK_VAULT_ROOT 环境变量
# 2. Windows: 使用 "D:/AI agent/tkk-library"
# 3. Linux/WSL: 使用 "/home/kangkang/tkk-library"
_VAULT_CANDIDATES = [
...
def grep_file_keywords(file_path, keywords):
    """Grep for multiple keywords at once"""
    try:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python batch_update_topic_index_fast.py <file_list>")
        sys.exit(1)

    file_list = sys.argv[1]
    files = Path(file_list).read_text(encoding='utf-8').strip().split("\n")
    total = len(files)
    print(f"Processing {total} files with {len(SEED_KEYWORDS)} keywords...")
    ...
    for i, rel_path in enumerate(files, 1):
        ...
        matched = grep_file_keywords(abs_path, SEED_KEYWORDS)
    ...
    total_sources = sum(t['source_count'] for t in topics.values())
    print(f"Done! {len(topics)} topics, {total_sources} source files indexed")
