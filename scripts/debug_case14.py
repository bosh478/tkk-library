...
import re
# Try without the opening ** on line 16
...
if match:
    ...
    # Now find what comes before this in the file
    before = content[:match.start()]
    # Find the last ** before the case name
    last_bold_start = before.rfind('**')
    print(f"\nLast ** before case name at: {last_bold_start}")
    print(f"Context: {repr(before[last_bold_start:])}")