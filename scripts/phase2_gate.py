...
def check_file(filepath):
    ...
    if not has_confidence:
        ...
        # 2b. Check for suspicious 100% high-confidence (v53)
        has_certain = '〔确定〕' in content
        has_doubtful = '〔存疑〕' in content
        has_to_verify = '〔待核实〕' in content
        if has_certain and not has_doubtful and not has_to_verify:
            issues.append(f"  [WARN] 置信度: 全部标为〔确定〕，无〔存疑〕/〔待核实〕，建议人工复核标注合理性")
